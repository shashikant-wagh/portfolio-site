...
(function ($) {
  ...
  function getSectionsOffsets() {
    var _sectionNodeList = document.querySelectorAll("section"),
      i = 0,
      sections = [],
      homeHeight = document.querySelector("#home")
      ...
      for (i in sections) {
        if (scrollPosition >= sections[i].offsetTop) {
          if ($('#navigation a[href="#' + sections[i].id + '"]').length) {
            ...
            $('#navigation a[href="#' + sections[i].id + '"]').addClass(
              "active"
            );
          }
        }
  ...
  function malat_pageCover() {
    ...
    if (pageCover.length > 0) {
      var pageCover_height = Math.max(
          $("#page-cover .page-cover-wrap").height(),
      ...
      $(window).on("scroll", function () {
        ...
        if (pageCover_height > scrollPos) {
          var opacity_value =
            (100 - (100 * scrollPos) / pageCover_height) / 100;
